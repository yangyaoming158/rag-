...
2026-06-16 调整：用户决定先搁置正式录屏，Phase 7 的 MVP 合并 Gate 调整为“交付材料完整 + 冷启动 + 浏览器端到端验收”。录屏保留为 Post-MVP 延后项，不阻塞合并到 `main`。
...
- **契约**：
  ...
  - 正式录屏不伪造；若当前环境无法录屏，保留为 Post-MVP 延后项
- **验收**：命令输出通过；容器最终状态符合用户要求；dev-log 记录做了什么/没做什么/遗留。
- **禁止**：不为了演示材料伪造录屏或篡改已执行证据。
...
| 录屏在当前环境不可完成 | 产出精确脚本和检查表，作为 Post-MVP 延后项 |
